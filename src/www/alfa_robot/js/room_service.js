    ...
    this.send = function(text) {
        if (text !== '') {
            ...
            try {
                ...
                var req;
                if (lang == 'en-EN' || (send_lang == 'eng')) {
                    req = window.config.spell_url + '/speak.php?args=-n zira&text= ' + split_text[0];
                } else {
                    req = window.config.spell_url + '/speak.php?args=-n milena&text= ' + split_text[0];
                }
...
InitRoomService = function(stt_config) {
    ...
    _server_socket.onerror = function (evt) {
        setTimeout(function(){
            console.log('!!! error');
            _server_socket = new WebSocket(window.config.robots_server_url)
        }, 100);
    ...
    _server_socket.onclose = function (evt) {
        setTimeout(function(){
            console.log('!!! close');
            _server_socket = new WebSocket(window.config.robots_server_url)
        }, 100);
