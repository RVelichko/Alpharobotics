    ...
    public:
        ...
        private:
            ...
            void send_from_queue() {
                auto msg_func = [this](const boost::system::error_code& ec, size_t /*bytes_transferred*/) {
                ...
                auto header_func = [this, msg_func](const boost::system::error_code& ec, size_t /*bytes_transferred*/) {
                    if(not ec) {
                        boost::asio::async_write(*socket, send_queue.begin()->message_stream->streambuf, strand.wrap(msg_func));
                    } else if (not send_queue.empty()) {
                        auto send_queued = send_queue.begin();
                        if (send_queued->callback) {
                            send_queued->callback(ec);
                        }
                ...
                auto post_func = [this, header_func]() {
                    boost::asio::async_write(*socket, send_queue.begin()->header_stream->streambuf, strand.wrap(header_func));
                };
                strand.post(post_func);
            }
    ...
    protected:
        ...
        void write_handshake(std::shared_ptr<Connection> connection, std::shared_ptr<boost::asio::streambuf> read_buffer) {
            ...
            for(auto& endp : opt_endpoint) {
                boost::smatch path_match;
                //std::cout << connection->path << "; " << endp.first << "\n" << std::flush;
